...
func TestCompanyService_Create(t *testing.T) {
	...
	svc := NewService(compRepo, actFieldRepo, logger)
	...
	testCases := []struct {
		...
		beforeTest func(compRepo mocks.MockICompanyRepository, actFieldRepo mocks.MockIActivityFieldRepository)
		wantErr    bool
		errStr     error
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.beforeTest != nil {
				tc.beforeTest(*compRepo, *actFieldRepo)
			}
...
func TestCompanyService_GetByOwnerId(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			companies, pages, err := svc.GetByOwnerId(ctx, tc.id, 1, true)
